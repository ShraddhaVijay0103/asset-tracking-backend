    ...
    public async Task<IActionResult> GetCompleteStatusBySite(Guid siteId)
    {
        ...
        foreach (var truck in trucks)
        {
            ...
            var truckPresent = await _db.GateEvents
                .AnyAsync(g => g.TruckId == truckId && g.SiteId == siteId);
            ...
            // =====================================================
            // ✅ GET ALL CLOSED CASES FOR THIS TRUCK
            // =====================================================
            var closedCaseIds = await _db.MissingEquipmentCases
                .Where(c => c.TruckId == truckId &&
                            c.SiteId == siteId &&
                            c.StatusId == 4)
                .Select(c => c.MissingEquipmentCaseId)
                .ToListAsync();

            var recoveredEquipmentIds = new HashSet<Guid>();

            if (closedCaseIds.Any())
            {
                recoveredEquipmentIds = new HashSet<Guid>(
                    await _db.MissingEquipmentCaseItems
                        .Where(i => closedCaseIds.Contains(i.MissingEquipmentCaseId)
                                    && i.SiteId == siteId
                                    && i.IsRecovered)
            ...
            if (lastEntryEvent != null)
            {
                entryEquipmentIds = await _db.GateEventItems
                    .Where(x => x.GateEventId == lastEntryEvent.GateEventId && x.SiteId == siteId)
                    .Select(x => x.EquipmentId)
                    .ToListAsync();

                entryEquipment = await _db.Equipment
                    .Where(e => entryEquipmentIds.Contains(e.EquipmentId))
            ...
            if (lastExitEvent != null)
            {
                exitEquipmentIds = await _db.GateEventItems
                    .Where(x => x.GateEventId == lastExitEvent.GateEventId && x.SiteId == siteId)
                    .Select(x => x.EquipmentId)
                    .ToListAsync();
            }
            ...
            var checkinTable = entryEquipment
                ...
                {
                    Equipment = e.Name,
                    GateStatus = recoveredEquipmentIds.Contains(e.EquipmentId)
                        ? "Complete"
                        : "MISSING",
                    EquipmentId = e.EquipmentId
                })
            ...
            results.Add(new
            {
                ...
                CheckOut = new
                {
                    ...
                    Summary = new
                    {
                        ...
                        TotalAssigned = exitEquipmentIds.Count
                    }
                },
                ...
                CheckIn = new
                {
                    ...
                    Summary = new
                    {
                        ...
                        TotalDetected = exitEquipmentIds.Count,
